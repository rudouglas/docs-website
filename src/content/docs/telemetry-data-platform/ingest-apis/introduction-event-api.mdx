...
1. You'll use a New Relic account [license key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key).
...
Alternatively, this API can use an [Insights insert key](/docs/apis/intro-apis/new-relic-api-keys/#insights-insert-key), but the license key is preferred. 
...
The `NrIntegrationError` event allows you to query and set alerts on custom data being sent to your New Relic account. **Recommendation:** To have New Relic Alerts notify you about parsing errors, [create a NRQL condition](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) for `NrIntegrationError`. Use this example NRQL query:
