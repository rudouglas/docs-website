...
You can find these errors in the `NrIntegrationError` event in the account associated with the key you used (either the [license key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) or the [Insights insert key](/docs/apis/intro-apis/new-relic-api-keys/#insights-insert-key)). The `requestId` for each request will be tagged on the `NrIntegrationError` event. See [Troubleshoot a `NRIntegrationError` event](/docs/troubleshoot-nrintegrationerror-event) for more information.
